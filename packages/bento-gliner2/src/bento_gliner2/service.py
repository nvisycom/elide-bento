...
The default implementation of the NER wire contract (``bento_core.ner.v1``). A
single GLiNER2 model serves the deployment (``ELIDE_BENTO_NER_MODEL``); each request
...
import bentoml
from bento_core.ner.v1 import NerRequest, NerResponse
...
image = bentoml.images.Image(python_version="3.12", lock_python_packages=False).requirements_file(
    "packages/bento-gliner2/requirements.txt"
)
...
@bentoml.service(
    ...
    # memory request when running with ELIDE_BENTO_NER_QUANTIZE.
