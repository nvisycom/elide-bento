...
VLM read. Implements the ``bento_core.ocrvl.v1`` contract.
...
import bentoml
from bento_core.ocrvl.v1 import Region, VlRequest, VlResponse
from bento_core.runtime import get_logger, request_id, resolve_model
from bentoml.exceptions import BentoMLException, InternalServerError, InvalidArgument
from prometheus_client import Histogram

from bento_paddleocr.block_map import block_kind

if TYPE_CHECKING:
    from PIL.Image import Image as PILImage

logger = get_logger("nvisy.vl")

# Built-in default model. Declared as the ELIDE_BENTO_MODEL_NAME env default below.
...
image = (
    ...
    .requirements_file("packages/bento-paddleocr/requirements.txt")
)


@bentoml.service(
    name="bento-paddleocr",
    image=image,
    # GPU service; a deployment without a GPU can set ELIDE_BENTO_DEVICE=cpu (slow).
    resources={"gpu": 1},
    traffic={"timeout": 120},
    envs=[
        {"name": "ELIDE_BENTO_MODEL_PATH", "value": "/models"},
        {"name": "ELIDE_BENTO_MODEL_NAME", "value": DEFAULT_MODEL},
        {"name": "ELIDE_BENTO_DEVICE", "value": "gpu"},
    ],
)
class OcrVlService:
    def __init__(self) -> None:
        ...
        device = os.getenv("ELIDE_BENTO_DEVICE", "gpu")
...
def _regions_from_result(result) -> list[Region]:
    ...
    """
    from bento_core.ocr.v1 import BoundingBox
