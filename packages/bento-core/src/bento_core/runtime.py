...
Kept dependency-light (no BentoML import) so ``bento-core`` stays a pure
...
MODEL_PATH_ENV = "ELIDE_BENTO_MODEL_PATH"
...
MODEL_NAME_ENV = "ELIDE_BENTO_MODEL_NAME"
...
def resolve_model() -> str:
    ...
    If ``$ELIDE_BENTO_MODEL_PATH`` points at a directory that contains weights, load
    those (mounted/BYO weights); otherwise load ``$ELIDE_BENTO_MODEL_NAME``. Both envs
    ...
    if not name:
        # Served bentos inject ELIDE_BENTO_MODEL_NAME; guard direct/test invocation.
...
def request_id(ctx: object) -> str | None:
    """Pull the correlation id from a BentoML request context, or ``None``.
    ...
    caller; absent it (or no headers at all), returns ``None`` rather than
    raising or inventing a sentinel.
    """
