...
The Rust runtime (``elide-bentoml``) is the source of truth; these
models mirror it. The wire is camelCase to match the runtime's serde
``rename_all = "camelCase"``. Split across :mod:`~bento_core.ner.v1.request` and
:mod:`~bento_core.ner.v1.response`; this package re-exports the public surface.
"""

from bento_core.ner.v1.request import (
    ClassificationSpec,
    EntitySpec,
    FieldSpec,
    NerRequest,
...
from bento_core.ner.v1.response import (
    Classification,
    ClassLabel,
    Entity,
    NerResponse,
