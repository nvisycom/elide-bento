...
import pytest
from bento_core.runtime import request_id, resolve_model
...
def test_populated_path_takes_precedence(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ELIDE_BENTO_MODEL_PATH", str(tmp_path))
    monkeypatch.setenv("ELIDE_BENTO_MODEL_NAME", "ignored/name")
...
def test_empty_path_falls_through_to_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ELIDE_BENTO_MODEL_PATH", str(tmp_path))  # exists but empty
    monkeypatch.setenv("ELIDE_BENTO_MODEL_NAME", "org/some-model")
    assert resolve_model() == "org/some-model"


def test_missing_path_uses_name(monkeypatch):
    monkeypatch.setenv("ELIDE_BENTO_MODEL_PATH", "/does/not/exist")
    monkeypatch.setenv("ELIDE_BENTO_MODEL_NAME", "org/some-model")
    assert resolve_model() == "org/some-model"


def test_raises_when_no_name_and_no_weights(monkeypatch):
    # Served bentos inject ELIDE_BENTO_MODEL_NAME; without it (and no mounted weights),
    # resolve_model has nothing to load and raises rather than guess.
    monkeypatch.delenv("ELIDE_BENTO_MODEL_PATH", raising=False)
    monkeypatch.delenv("ELIDE_BENTO_MODEL_NAME", raising=False)
    with pytest.raises(RuntimeError):
        resolve_model()


def test_request_id_none_without_headers():
    assert request_id(object()) is None
